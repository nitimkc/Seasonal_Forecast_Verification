...
interval <- 
PlotEquiMap(PrNovStartData$mod[1,1,1,1,,], NovLon, NovLat,
            toptitle = "MF_sea", sizetit = 0.5,
            brks = interval, cols = color, 
            drawleg = FALSE, numbfig = 4, axelab = FALSE,
            #labW = FALSE, intylat = 20, intxlon = 20,
            filled.continents = FALSE, )#square = TRUE, dots=t(dot1))
...
PlotEquiMap(PrNovStartData$mod[1,1,1,1,,], NovLon, NovLat,
            toptitle = "MF_sea_mask", sizetit = 0.5,
            brks = interval, cols = color, 
            drawleg = FALSE, numbfig = 4, axelab = FALSE,
            #labW = FALSE, intylat = 20, intxlon = 20,
            filled.continents = FALSE, )#square = TRUE, dots=t(dot1))
...
# prepare data
corrDJFm <- melt(corrDJF)
colnames(corrDJFm) <- c("Var1", "Var2", "ValueGroup", "lat", "lon", "value")

# map lat/lon
lat <-hash(unique(corrDJFm$lat), NovLat)
lon <-hash(unique(corrDJFm$lon), NovLon)

corrDJFm[,"lat"] <- sapply(corrDJFm$lat, function(x){return(lat[[as.character(x)]])})                      
corrDJFm[,"lon"] <- sapply(corrDJFm$lon, function(x){return(lon[[as.character(x)]])})                      

corrOnly <- as.data.frame(corrDJFm[which(corrDJFm$ValueGroup == 3), ]) 
corrOnly <- corrOnly[which(corrOnly == 1),] #for mod 1


#col <- c('#67001f', '#b2182b', '#d6604d', '#f4a582', '#fddbc7', #'#f7f7f7', '#ffffff', 
